...
const Mycourt = () => {
    ...
    useEffect(() => {
        const getMyCourt = async () => {
            try {
                if (user && user._id) {
                    const mycourt = await axios.get(`https://newbackend-176c.onrender.com/court/mycourt/${user._id}`)
                    // const mycourt = await axios.get(`https://newbackend-176c.onrender.com/court/mycourt/${user._id}`)
    ...
    const CreateShedule = async (court) => {
        ...
        if (DateRangeState.startDate) {
            if (selectedSlots.length > 0) {
                try {
                    const createSlot = await axios.post(`https://newbackend-176c.onrender.com/Slot/${court._id}`,
                        {
